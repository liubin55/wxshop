...
<div>
        <div class="g-pay-lst">
            <ul>
                @foreach($data as $v)
                <li class="orders" goods_id="{{$v->goods_id}}">
                    <a href="">
                        <span>
                        ...
                        <dl>
                            ...
                            <dd>价格:<em >￥{{$v->self_price}}</em>&nbsp;&nbsp;数量:<em>{{$v->buy_number}}</em>&nbsp;&nbsp;小计：￥<em class="price">{{$v->buy_number*$v->self_price}}</em></dd>
                        </dl>
                    </a>
                </li>
                    @endforeach
...
<script>
	...
	$(document).ready(function(){
	    ...
	    $("#btnPay").click(function () {
            ...
            $.ajax({
                type:'post',
                data:{goods_id:goods_id,_token:$("#_token").val()},
                url:"{{url('orderform')}}",
                dataType:'json'
            }).done(function (res) {
                if(res.code==1){
